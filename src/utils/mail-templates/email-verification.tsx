...
const VerifyEmail: React.FC<Readonly<VerifyEmailProps>> = ({
  otp,
  fullName,
  expiryMinutes = 10,
  ...
  return (
    <Html lang="en">
      <Head>
        <title>vCare Project Management Verification Code</title>
      </Head>
      <Container style={{ fontFamily: "Arial, sans-serif", lineHeight: "1.6", color: "#111827", padding: "12px" }}>
        <Text>Hello {fullName?.trim() || "user"},</Text>
        <Text>To complete your account registration, please verify using the verification code below:</Text>
        <Text>
          OTP Code: <strong>{otp}</strong>
        </Text>
        <Text>
          This code is valid for <strong>{expiryMinutes}</strong> minutes.
        </Text>
